  ...
  template: `
    <div class="container mx-auto p-4">
      ...
      <div *ngIf="!isLoading && !movies.length" class="card bg-base-200 p-8 text-center">
        <p class="mb-4">You haven't marked any movies as watched yet. Start watching something from your watchlist!</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6" *ngIf="movies.length">
        <div class="card lg:card-side bg-base-300 shadow-xl relative" *ngFor="let movie of movies">
          ...
          <div class="card-body">
            ...
            <a [routerLink]="['/movie', movie.id]" class="btn btn-outline btn-sm absolute top-2 right-2 z-10">Details</a>
            <div class="card-actions flex-col gap-2 mt-4 items-center">
              <div class="grid grid-cols-2 gap-2 w-full">
                <button (click)="updateStatus(movie.documentId!, 'want-to-watch')" class="btn btn-primary btn-sm">
                  Move to Watchlist
                </button>
                <button (click)="updateStatus(movie.documentId!, 'in-progress')" class="btn btn-warning btn-sm">
                  Mark In Progress
                </button>
              </div>
              <button (click)="removeFromTracking(movie.documentId!)" class="btn btn-error btn-sm w-full">
                Remove
              </button>
            </div>
          </div>
...
export class WatchedComponent implements OnInit {
  ...
  async updateStatus(movieId: string, newStatus: 'want-to-watch' | 'in-progress'): Promise<void> {
    const success = await this.databaseService.updateMovieStatus(movieId, newStatus);
    if (success) {
